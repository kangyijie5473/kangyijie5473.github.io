...
date: 2017-07-28
tags: [Network, Algorithm, C/C++, Linux]
description: "基于C++11实现的半同步-半异步线程池源码分析第二部分：线程池（ThreadPool）。详细讲解了线程池的启动、停止、任务添加以及工作线程的运行逻辑，涉及 `std::thread`, `std::function` 等C++11特性。"
---
基于C++11实现的半同步-半异步线程池源码分析第二部分：线程池（ThreadPool）。详细讲解了线程池的启动、停止、任务添加以及工作线程的运行逻辑，涉及 `std::thread`, `std::function` 等C++11特性。
