...
date: 2018-06-08
tags: [Linux, Tips]
description: "本文详细介绍了在Linux下通过源码编译安装软件的流程（configure, make, make install）。重点讲解了如何使用 `--prefix` 指定安装目录以避免权限问题，以及利用 `pkg-config` 管理库依赖（如头文件路径和链接库路径），从而简化开发时的编译链接配置。"
---
本文详细介绍了在Linux下通过源码编译安装软件的流程（configure, make, make install）。重点讲解了如何使用 `--prefix` 指定安装目录以避免权限问题，以及利用 `pkg-config` 管理库依赖（如头文件路径和链接库路径），从而简化开发时的编译链接配置。
