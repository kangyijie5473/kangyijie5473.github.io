...
date: 2017-10-18
tags: [Tools, Redis, Network, C/C++, AI]
description: 网络编程定时事件处理系列第一篇。模仿Redis服务端的实现，使用链表维护定时器，并在主循环中结合epoll_wait处理IO事件和定时事件，提供了一个C++实现的简单Demo。
---
网络编程定时事件处理系列第一篇。模仿Redis服务端的实现，使用链表维护定时器，并在主循环中结合epoll_wait处理IO事件和定时事件，提供了一个C++实现的简单Demo。
