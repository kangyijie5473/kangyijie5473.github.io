...
date: 2017-10-23
tags: [Redis, Network, C/C++, Python, AI, Linux]
description: 网络编程定时事件处理系列第二篇。介绍利用 `alarm` 函数和信号（Signal）机制来实现定时器，并通过管道（Pipe）统一事件源，使 `epoll_wait` 能够同时监听IO事件和信号事件。
---
网络编程定时事件处理系列第二篇。介绍利用 `alarm` 函数和信号（Signal）机制来实现定时器，并通过管道（Pipe）统一事件源，使 `epoll_wait` 能够同时监听IO事件和信号事件。
