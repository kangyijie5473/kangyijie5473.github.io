...
date: 2017-09-20
tags: [Tools, Redis]
description: 深入探究Redis字符串对象 `embstr` 与 `raw` 编码的分界值变化。分析了从Redis 3.0的39字节变为Redis 3.2+的44字节的原因，涉及 `sdshdr` 结构的内存优化。
---
深入探究Redis字符串对象 `embstr` 与 `raw` 编码的分界值变化。分析了从Redis 3.0的39字节变为Redis 3.2+的44字节的原因，涉及 `sdshdr` 结构的内存优化。
...
根据知乎上的回答怀疑在3.0和3.2版本之间的改动导致了39字节不再适用，于是直接在github查看object.c的改动。同时也希望能找到为什么是39而不是其他数。
