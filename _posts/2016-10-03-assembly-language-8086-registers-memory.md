...
date: 2016-10-03
tags: [Network, C/C++]
description: "本文是《汇编语言》学习笔记的第一部分，基于8086 CPU架构。主要介绍了汇编指令与伪指令的区别，通用寄存器（AX, BX等）的用途，段寄存器（CS, DS, SS）与指针寄存器（IP, SP）在内存寻址（段地址+偏移地址）中的核心作用，以及栈操作（push/pop）的基本原理。"
---
本文是《汇编语言》学习笔记的第一部分，基于8086 CPU架构。主要介绍了汇编指令与伪指令的区别，通用寄存器（AX, BX等）的用途，段寄存器（CS, DS, SS）与指针寄存器（IP, SP）在内存寻址（段地址+偏移地址）中的核心作用，以及栈操作（push/pop）的基本原理。
...
寄存器CS（Code Segment代码段寄存器） IP（Instruction
