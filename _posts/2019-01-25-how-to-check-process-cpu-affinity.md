...
date: 2019-01-25
tags: [Network, AI, Linux]
description: "本文介绍了在Linux系统中查看进程运行在哪个CPU核心上的两种方法。一种是通过命令行工具 `ps -o psr -p <pid>` 直接查看；另一种是在C程序中调用 `sched_getcpu()` 函数或读取 `/proc/<pid>/stat` 文件（解析第39个字段）来获取当前运行的CPU核心编号。"
---
本文介绍了在Linux系统中查看进程运行在哪个CPU核心上的两种方法。一种是通过命令行工具 `ps -o psr -p <pid>` 直接查看；另一种是在C程序中调用 `sched_getcpu()` 函数或读取 `/proc/<pid>/stat` 文件（解析第39个字段）来获取当前运行的CPU核心编号。
