...
date: 2017-05-20
tags: [Tools, Life, Network, C/C++]
description: 翻译自GCC手册，介绍了系统特定的预定义宏（System-specific Predefined Macros）。解释了GCC如何处理系统特定宏的命名空间，以及并行宏（如 `__unix__`）的使用。
---
翻译自GCC手册，介绍了系统特定的预定义宏（System-specific Predefined Macros）。解释了GCC如何处理系统特定宏的命名空间，以及并行宏（如 `__unix__`）的使用。
