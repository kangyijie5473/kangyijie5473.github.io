...
date: 2017-04-16
tags: [C/C++, AI, Linux]
description: 解析Linux内核中常用的 `container_of` 宏的实现原理。详细解释了如何利用 `typeof` 和 `offsetof` 根据结构体成员的地址获取结构体的首地址，以及其中指针类型转换的细节。
---
解析Linux内核中常用的 `container_of` 宏的实现原理。详细解释了如何利用 `typeof` 和 `offsetof` 根据结构体成员的地址获取结构体的首地址，以及其中指针类型转换的细节。
