...
date: 2016-07-27
tags: [C/C++, AI, Linux]
description: "本文探讨了如何利用函数指针数组（跳转表）来简化繁琐的 `switch-case` 结构。以计算器程序为例，展示了如何将加减乘除运算封装为结构统一的函数，并通过字符串映射找到对应的函数指针进行调用，从而优化代码结构。"
---
本文探讨了如何利用函数指针数组（跳转表）来简化繁琐的 `switch-case` 结构。以计算器程序为例，展示了如何将加减乘除运算封装为结构统一的函数，并通过字符串映射找到对应的函数指针进行调用，从而优化代码结构。
