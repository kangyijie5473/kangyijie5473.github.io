...
date: 2018-01-24
tags: [Algorithm, C/C++]
description: "本文解析了LintCode 181题“将整数A转换为B”。主要考察位运算，通过异或操作找出不同的位，再统计1的个数。文章重点分析了负数右移运算在C/C++中的实现依赖性（算术右移通常补符号位），解释了由此导致的死循环问题及其解决方案。"
---
本文解析了LintCode 181题“将整数A转换为B”。主要考察位运算，通过异或操作找出不同的位，再统计1的个数。文章重点分析了负数右移运算在C/C++中的实现依赖性（算术右移通常补符号位），解释了由此导致的死循环问题及其解决方案。
