...
date: 2017-09-30
tags: [Linux, Redis, Network, Python, Life]
description: 分析Redis服务端的设计模型。介绍了Redis采用的单线程Reactor模型，以及如何通过封装 `ae` 事件库来实现跨平台的I/O多路复用（如epoll, kqueue, select）。
---
分析Redis服务端的设计模型。介绍了Redis采用的单线程Reactor模型，以及如何通过封装 `ae` 事件库来实现跨平台的I/O多路复用（如epoll, kqueue, select）。
