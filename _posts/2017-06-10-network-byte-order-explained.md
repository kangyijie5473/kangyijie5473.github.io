...
date: 2017-06-10
tags: [Network, Life, C/C++, Linux]
description: 总结网络编程中的字节序问题。解释主机字节序（通常是小端）与网络字节序（大端）的区别，以及如何使用 `htonl` 等函数进行转换，避免数据传输错误。
---
总结网络编程中的字节序问题。解释主机字节序（通常是小端）与网络字节序（大端）的区别，以及如何使用 `htonl` 等函数进行转换，避免数据传输错误。
