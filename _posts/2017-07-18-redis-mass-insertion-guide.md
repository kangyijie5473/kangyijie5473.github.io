...
date: 2017-07-18
tags: [Redis, Python, Network, Life]
description: 介绍如何使用Redis Pipe模式进行批量数据导入。以英文单词-汉译数据为例，对比了普通循环插入和Pipe模式插入的效率，并给出了Python脚本生成Redis协议格式数据的示例。
---
介绍如何使用Redis Pipe模式进行批量数据导入。以英文单词-汉译数据为例，对比了普通循环插入和Pipe模式插入的效率，并给出了Python脚本生成Redis协议格式数据的示例。
