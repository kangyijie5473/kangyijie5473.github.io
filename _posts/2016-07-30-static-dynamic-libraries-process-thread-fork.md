...
date: 2016-07-30
tags: [Linux, C/C++, Python, AI, Life]
description: "本文简要介绍了C语言开发中的静态库与动态库的区别及制作方法，并探讨了进程与线程的基本概念。重点分析了 `fork` 创建子进程时的写时复制（COW）机制，以及标准输出缓冲区在父子进程间复制导致的重复打印问题。"
---
本文简要介绍了C语言开发中的静态库与动态库的区别及制作方法，并探讨了进程与线程的基本概念。重点分析了 `fork` 创建子进程时的写时复制（COW）机制，以及标准输出缓冲区在父子进程间复制导致的重复打印问题。
