...
date: 2018-01-22
tags: [Algorithm]
description: "本文解析了LintCode 158题“两个字符串是变位词”的解法。对比了直接排序（时间复杂度较高）和哈希表/桶排序（统计字符频次，时间O(n)空间O(1)）两种思路。此外，作者还探讨了OJ平台上相同代码多次提交执行时间差异的问题。"
---
本文解析了LintCode 158题“两个字符串是变位词”的解法。对比了直接排序（时间复杂度较高）和哈希表/桶排序（统计字符频次，时间O(n)空间O(1)）两种思路。此外，作者还探讨了OJ平台上相同代码多次提交执行时间差异的问题。
