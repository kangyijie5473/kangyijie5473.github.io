...
date: 2016-07-20
tags: [Life, C/C++, Linux]
description: "本文继续探讨Linux系统编程中的文件操作，主要包括使用 `fcntl` 函数控制文件描述符属性（特别是文件记录锁 `F_SETLK` 等），以及实现一个简易的 `ls -Rl` 命令（`my_ls`）。文章详细解释了文件记录锁的原理和代码实现，并展示了如何遍历目录和递归显示文件信息。"
---
本文继续探讨Linux系统编程中的文件操作，主要包括使用 `fcntl` 函数控制文件描述符属性（特别是文件记录锁 `F_SETLK` 等），以及实现一个简易的 `ls -Rl` 命令（`my_ls`）。文章详细解释了文件记录锁的原理和代码实现，并展示了如何遍历目录和递归显示文件信息。
