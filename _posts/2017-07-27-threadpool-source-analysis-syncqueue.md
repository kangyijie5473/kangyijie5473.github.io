...
date: 2017-07-27
tags: [Network, Tools, C/C++, Linux]
description: "基于C++11实现的半同步-半异步线程池源码分析第一部分：任务队列（SyncQueue）。详细讲解了使用 `std::mutex` 和 `std::condition_variable` 实现线程安全的同步队列。"
---
基于C++11实现的半同步-半异步线程池源码分析第一部分：任务队列（SyncQueue）。详细讲解了使用 `std::mutex` 和 `std::condition_variable` 实现线程安全的同步队列。
