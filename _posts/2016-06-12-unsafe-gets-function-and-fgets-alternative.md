...
date: 2016-06-12
tags: [Tools, Redis, Network, C/C++, AI, Linux]
description: "本文讨论了C语言中不安全的 `gets()` 函数及其替代方案。作者指出 `gets()` 无法限制输入长度，容易导致缓冲区溢出，建议使用更安全的 `fgets()` 函数，并详细演示了 `fgets()` 的用法和换行符处理细节。"
---
本文讨论了C语言中不安全的 `gets()` 函数及其替代方案。作者指出 `gets()` 无法限制输入长度，容易导致缓冲区溢出，建议使用更安全的 `fgets()` 函数，并详细演示了 `fgets()` 的用法和换行符处理细节。
...
tips:关于C99 啥的可以看一下ANSI C我们先来看一下函数原型
...
1. 函数参数为10，当我们输入超过了10个字符，由输出可知，str数组中存放了kangkangh9个字符加一个NULL
