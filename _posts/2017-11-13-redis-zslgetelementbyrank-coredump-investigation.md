...
date: 2017-11-13
tags: [Redis, Network, AI, Life]
description: 记录一次排查Redis跳跃表相关Bug的经历。在使用 `zslGetElementByRank` 函数时遇到coredump，通过切换内存分配器（jemalloc vs libc）发现问题根源，涉及结构体成员访问异常。
---
记录一次排查Redis跳跃表相关Bug的经历。在使用 `zslGetElementByRank` 函数时遇到coredump，通过切换内存分配器（jemalloc vs libc）发现问题根源，涉及结构体成员访问异常。
